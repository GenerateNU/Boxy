...
export default function ReservationOverview(reservation: any, currentForm: number, setCurrentForm: Function) {
    return (
            ...
            <div className='border-[#B5B5B5] border-[0.75px] rounded-md p-[26px]'>
                ...
                <div className='w-[100%] flex place-content-between mb-3 mt-3'>
                    ...
                    <div className='flex flex-col text-right'>
                        <h4 className='text-[10px] md:text-[15px]'>{reservation.address}</h4>
                        <h4 className='text-[10px] md:text-[15px]'>{reservation.city + ', ' + reservation.state}</h4>
                    </div>
                </div>
                <hr className='w-[100%] h-[0.75px] bg-[#B5B5B5] border-0'/>
                <div className='w-[100%] flex place-content-between mb-3 mt-3'>
                ...
                {reservation.protection != null ? 
                    <>
                        ...
                        <div className='w-[100%] flex place-content-between mb-3 mt-3'>
                            ...
                            <h4 className='text-[10px] md:text-[15px]'>{reservation.protection ? 'Yes' : 'No'}</h4>
                        </div>
                    </>
                    : <></>}
                {reservation.items != null ? 
                    <>
                        ...
                        <h4 className='text-[10px] md:text-[15px] text-[#B5B5B5] mb-3'>{reservation.items}</h4>
                    </> 
                    : <></>}
                    ...
                    <>
                        ...
                        <div className='grid grid-cols-3 gap-2'>
                            {reservation.images.map((image: string) => {
                                return <img className='object-cover w-[7.5vw] h-[7.5vw]' src={image}/>
                            })}
                        </div> 
                    </>
                    : <></>}
            </div>
            <button onClick={() => setCurrentForm(currentForm + 1)} className={`h-[60px] w-[25vw] lg:w-[25vw] mt-5 bg-bxBrand text-white rounded-full hover:bg-bxBrandLight transition ease-in duration-75`} >
                Continue
            </button>
        </div>
    )
