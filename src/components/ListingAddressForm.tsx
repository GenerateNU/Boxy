export default function ListingAddressForm(props: any) {
  const create_input = (placeholder: string) => {
    return (
      <input
        onChange={(event) =>
          props.updateListingAttribute(placeholder, event.target.value)
        }
    ...
    <div className="container min-w-full flex flex-col items-center mt-[10vh] ">
      ...
      {/* <div className="absolute bottom-10 w-[80%]">
        <div className="flex justify-between">
          <div className="">
            <button className="border border-solid border-black h-[5vh] w-[8vw] mb-7 right-2 rounded-full text-black">
              Back
            </button>
          </div>
          <div className="">
            <button className="bg-[#7C7C7C] h-[5vh] w-[8vw] mb-7 right-2 rounded-full text-white">
              Next
            </button>
          </div>
        </div>
